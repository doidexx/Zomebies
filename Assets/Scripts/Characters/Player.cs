...
public class Player : MonoBehaviour
{
    ...
    private void ScrollWeapon()
    {
        ...
        int weaponIndexer = Mathf.Clamp(currentWeaponIndex + (int)Input.mouseScrollDelta.y, 0, weapons.Length - 1);
        if (weaponIndexer == currentWeaponIndex) return;
        if (weapons[weaponIndexer] == null) return;
        print(weaponIndexer);
        weapons[currentWeaponIndex].PutAway();
        currentWeaponIndex = weaponIndexer;
    }
    ...
    private void Movement()
    {
        ...
        if (forward != 0 || horizontal != 0 && running == false)
            moving = true;
        else
            moving = false;
    }
